...
## ★ 接手協定(最優先)
**當 user 說「幫我繼續下去 / 繼續工作下去 / 繼續 / 接手 / 接著做」之類的話,代表 Claude(主席)不在了,你立刻升任代理主席:**
1. 讀同目錄的 **`HANDOFF.md`**(接力棒,記著做到哪、下一步清單)。
2. 挑 HANDOFF.md「下一步 backlog」最上面那個未劃掉的項目開始做。
3. 全程遵守下面所有 Hard Rule(尤其:改完必實測才算修好、你不能自審改用 `gemini -p`)。
4. 做完一項:更新 HANDOFF.md(劃掉完成的 + 補狀態)→ commit/push → 一句話報告 user + live URL。
5. 不用問 user 選哪個、不用等確認,自己拍最合理的順序往下做(重大不可逆才停)。
